...
# UI DESIGN: Professional Navy Blue Theme
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    # DYNAMIC TOGGLE: Select/Deselect All
...
if filtered_prices.empty or not selected_stocks:
    st.warning("⚠️ Please select stocks and years from the sidebar.")
    st.stop()
...
# UI: HEADER
watchlist_name = selected_file.replace(".xlsx", "")
st.title(f"📈 {watchlist_name}")
st.caption(f"📅 View Range: {filtered_prices.index.min().date()} to {filtered_prices.index.max().date()}")
...
t1, t2, t3, t4 = st.tabs(["📊 Visuals", "📋 Stats Data", "📅 Monthly", "🏢 Quarterly"])
...
with t4:
    ...
    # Robust year filtering to fix the Attribute Error
