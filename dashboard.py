...
with st.sidebar:
    ...
    selected_file = st.selectbox("Select Watchlist", files, key="main_file_selector")
    file_path = os.path.join(folder, selected_file)
    
    if st.button("🔄 Reload & Sync Data", use_container_width=True, key="sync_btn"):
        st.cache_data.clear()
        st.rerun()
    ...
    st.markdown("---")
    select_all = st.toggle("Select All Stocks", value=True, key="sel_all_toggle")
    current_default = all_stocks if select_all else []
    selected_stocks = st.multiselect("Active Stocks", all_stocks, default=current_default, key="active_stocks_sel")

    st.markdown("---")
    horizon = st.radio("⏱️ Time Horizon", ["Last 12 Months", "Last 6 Months", "Last 3 Months", "All Time", "Custom Years"], index=0, key="horizon_radio")
    
    selected_years = []
    if horizon == "Custom Years":
        available_years = sorted(prices_df.index.year.unique(), reverse=True)
        selected_years = st.multiselect("Select Years", available_years, default=available_years[:1], key="year_sel")
    ...
    if sel_months:
        ...
        with chart_col:
            if sel_stocks_chart:
                ...
                st.plotly_chart(px.line(cum_trend, template="plotly_white", markers=True, height=450), use_container_width=True)
        ...
        st.download_button(label="📥 Download Price History (CSV)", data=csv_data, file_name="stock_data.csv", mime='text/csv', use_container_width=True, key="csv_dl")
